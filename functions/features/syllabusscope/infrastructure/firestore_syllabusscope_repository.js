...
} = require("../../../schema/syllabusscope_schema");
...
} = require("../domain/syllabusscope_repository");
...
function normalizeLevels(levels = {}) {
  ...
  Object.entries(source).forEach(([levelType, grades]) => {
    if (!LEVEL_TYPES.has(levelType)) {
      throw new Error(
        `level type must be one of: ${syllabusScopeLevelTypes.join(", ")}.`,
      );
    }
    ...
    Object.entries(gradeSource).forEach(([gradeNumber, enabled]) => {
      if (!GRADE_NUMBERS.has(gradeNumber)) {
        throw new Error(
          `grade number must be one of: ${syllabusScopeGradeNumbers.join(", ")}.`,
        );
      }
...
class FirestoreSyllabusScopeRepository extends SyllabusScopeRepository {
  constructor({
  ...
  async findByCountry(country) {
    ...
    const syllabusScopes = await this.readCollection(
      SYLLABUS_SCOPE_COLLECTION,
    ...
    return syllabusScopes
      .map((data) => {
        try {
          ...
          console.warn(
            "Skipping invalid syllabus scope document.",
            data?.id,
            error,
          );
          return null;
